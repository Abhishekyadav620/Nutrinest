...
export default function Profile() {
  ...
  // Save profile changes (Only Email is synced with backend for now)
  const handleSaveProfile = async (payload) => {
    ...
    // If email changed, call API
    if (payload.email !== adminData.email) {
        const result = await updateProfile(payload.email);
        if (!result.success) {
  ...
  const handleChangePassword = async ({ oldPassword, newPassword }) => {
    // Note: Backend currently doesn't verify oldPassword, but we could add it.
    // For now, we trust the authenticated session.
    const result = await updateProfile(adminData.email, newPassword);
          ...
          <div className="d-flex flex-column flex-md-row gap-4">
            <div className="text-center text-md-start">
              ...
              <div className="d-none d-md-block mt-2">
                <h3 className="mb-0 fw-bold">{adminData.name}</h3>
                <p className="text-muted mb-0 mt-1">{adminData.role}</p>
              </div>
            </div>
            ...
            <div className="flex-fill">
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <h3 className="d-md-none mb-0 fw-bold">{adminData.name}</h3>
                  <p className="text-muted d-md-none mt-1">{adminData.role}</p>
                </div>
              </div>
